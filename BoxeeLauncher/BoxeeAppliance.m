...
#import "BoxeeAppliance.h"
#import "BoxeeController.h"
#import "BoxeeUpdateController.h"
...
+ (void) initialize
{
	...
	[defaultValues setValue:[NSNumber numberWithBool:YES] forKey:Boxee_USE_INTERNAL_IR];
	[defaultValues setValue:[NSNumber numberWithBool:YES] forKey:Boxee_USE_UNIVERSAL_REMOTE];
	
	//register dictionary defaults
	[[BoxeeUserDefaults defaults] registerDefaults:defaultValues];
}
...
- (id)controllerForIdentifier:(id)identifier {
	...
	//entry type is the key if this is an Boxee.app entry or something else like updater, etc
	if( [entry_type isEqualToNumber:[NSNumber numberWithInt: APPLICATION]] ){
		//there can be more than one boxee entry in the list, e.g. to test developer version etc.
		...
		return [[[BoxeeController alloc] initWithAppPath:appPath helperPath:helperPath lauchAgentFileName:launch_agent_file_name] autorelease];
	} 
		...
		return [[[BoxeeUpdateController alloc] initWithURL:url] autorelease];
	} else if( [identifier isEqualToString:@"Settings"] ){
		return [[[BoxeePreferencesController alloc] init] autorelease];
	} else {
		return [BRAlertController alertOfType:0
														titled:@"Boxee Launcher"
											 primaryText:@"Error"
