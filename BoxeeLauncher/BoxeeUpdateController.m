...
#import "BoxeeUpdateBlockingController.h"
#import "QuDownloadController.h"

@class BRLayerController;
@implementation BoxeeUpdateController


- (id) init {
	[self dealloc];
	@throw [NSException exceptionWithName:@"BNRBadInitCall" reason:@"Init BoxeeUpdateController with initWithURL" userInfo:nil];
	return nil;
}
...
- (void) wasExhumedByPoppingController: (id) controller
{
	...
	if(controller == mp_downloader){
		if ( [mp_downloader downloadComplete] ){
			...
			BoxeeUpdateBlockingController* blocker = [[[BoxeeUpdateBlockingController alloc] 
