...
{
	[self dealloc];
	@throw [NSException exceptionWithName:@"BNRBadInitCall" reason:@"Init BoxeeController with initWithPath" userInfo:nil];
	return nil;
}
- (id) initWithAppPath:(NSString*) f_app_path helperPath:(NSString*) f_helper_path lauchAgentFileName:(NSString*) f_lauch_agent_file_name
{
	...
	if ( ![super init] )
		return ( nil );
	mp_boxeeclient = [[BoxeeClientWrapper alloc] init];
	m_boxee_running = NO;
	...
	//read preferences
	m_use_internal_ir = [[BoxeeUserDefaults defaults] boolForKey:Boxee_USE_INTERNAL_IR];
	return self;
}
...
- (void)checkTaskStatus:(NSNotification *)note
{
	...
	if (![mp_task isRunning])
	{
		...
		//again try to set boxeehelper status what we want to have (in case it was changed during this session)
		[self setDesiredAppleRemoteMode];
		//try to kill BoxeeHelper (it does not hurt if it's not running, but definately helps if it still is
		...
		if (status != 0)
		{
			BRAlertController* alert = [BRAlertController alertOfType:0 titled:nil
																										primaryText:[NSString stringWithFormat:@"Error: Boxee exited With Status: %i",status]
																									secondaryText:nil];
...
- (bool) inUserSettingsSetXpath:(NSString*) f_xpath toInt:(int) f_value{
	...
	NSString* guisettings_path = [[app_support_path_array objectAtIndex:0] stringByAppendingString:@"/BOXEE/UserData/guisettings.xml"];
...
- (void) wasPushed{
	...
	//if enabled start our own instance of BoxeeHelper
	if( m_use_internal_ir ){
		//try to disable boxeehelper in guisettings.xml
	...
	@catch (NSException* e) {
		...
		BRAlertController* alert = [BRAlertController alertOfType:0 titled:nil
																									primaryText:[NSString stringWithFormat:@"Error: Cannot launch Boxee from path:"]
...
- (BOOL)brEventAction:(BREvent *)event
{
	if( m_boxee_running ){
		...
		DLOG(@"BoxeeController: Button press hashVal = %i",hashVal);
		switch (hashVal)
		{
			case 65676:  // tap up
				if([event value] == 1)
					[mp_boxeeclient handleEvent:ATV_BUTTON_UP_PRESS];
				else
					[mp_boxeeclient handleEvent:ATV_BUTTON_UP_RELEASE];
				return YES;
			case 65677:  // tap down
				if([event value] == 1)
					[mp_boxeeclient handleEvent:ATV_BUTTON_DOWN_PRESS];
				else
					[mp_boxeeclient handleEvent:ATV_BUTTON_DOWN_RELEASE];
				return YES;
			case 65675:  // tap left
				[mp_boxeeclient handleEvent:ATV_BUTTON_LEFT];
				return YES;
			case 65674:  // tap right
				[mp_boxeeclient handleEvent:ATV_BUTTON_RIGHT];
				return YES;
			case 65673:  // tap play
				[mp_boxeeclient handleEvent:ATV_BUTTON_PLAY];
				return YES;
			case 786611: //hold right
				[mp_boxeeclient handleEvent:ATV_BUTTON_RIGHT_H];
				return YES;
			case 786612: //hold left
				[mp_boxeeclient handleEvent:ATV_BUTTON_LEFT_H];
				return YES;
			case 65670: //menu
				[mp_boxeeclient handleEvent:ATV_BUTTON_MENU];
				return YES;
			case 786496: //hold menu
				[mp_boxeeclient handleEvent:ATV_BUTTON_MENU_H];
				return YES;
			default:
				ELOG(@"BoxeeController: Unknown button press hashVal = %i",hashVal);
				return NO;
		}
	} else {
		DLOG(@"Boxee not running. IR event goes upstairs");
...
- (void) killHelperApp:(NSTimer*) f_timer{
	...
	NSString* killer_path = [[NSBundle bundleForClass:[self class]] pathForResource:@"killHelper" ofType:@"sh"];
...
- (BOOL) setDesiredAppleRemoteMode
{
	if( m_use_internal_ir ){
		...
		if ([[BoxeeUserDefaults defaults] boolForKey:Boxee_USE_UNIVERSAL_REMOTE])
