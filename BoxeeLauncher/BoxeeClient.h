#ifndef __Boxee_CLIENT_H__
#define __Boxee_CLIENT_H__
...
public:
  ...
  bool Send(int Socket, CAddress &Addr, unsigned int UID = BoxeeClientUtils::GetUniqueIdentifier())
  {
    if (m_Payload.size() == 0)
      ConstructPayload();
...
private:
  ...
  unsigned int  m_UID;
public:
  CBoxeeClient(const char *IP = "127.0.0.1", int Port = 9777, int Socket = -1, unsigned int UID = 0)
  {
    ...
    if (UID)
      ...
      m_UID = BoxeeClientUtils::GetUniqueIdentifier();
  }
