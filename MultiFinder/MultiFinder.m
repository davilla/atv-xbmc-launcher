...
- (void) startApplicationRequest:(NSNotification *) notification {
    ...
    if ( [[userInfo objectForKey:kApplicationNeedsIR] boolValue] ) {
        if ([userInfo objectForKey:kApplicationWantsUniversalIRMode] &&
            [[userInfo objectForKey:kApplicationWantsUniversalIRMode] boolValue]) {
...
- (void) changeDefaultApplicationRequest:(NSNotification *) notification {
    ...
    DLOG(@"Got an change default application request for app %@ with args %@ withRemote:%i",
        default_app, default_app_args, ir_mode);
    //change default app
